...
const AlterBlog = () => {
    ...
    const onSubmitHandler = async(e)=>{
        try {
            ...
            const updates = {title, subtitle: subTitle, description: quillRef.current.root.innerHTML, category, is_published: isPublished}

            const formData = new FormData();
            formData.append('blog', JSON.stringify(updates))
            formData.append('image', image)

            const {data} = await axios.patch('/api/blog/update', formData);
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
        <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
            ...
            <label htmlFor="image">
                <img  src={!image ? assets.upload : URL.createObjectURL(image)} alt="" className='mt-2 h-16 rounded cursor-pointer outline-1 ouline-dotted'/>
